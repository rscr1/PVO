...
class PanopticFPN_video_DatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        for k,v in ret.items():    
            ...
            sem_seg_gt = utils.read_image(dataset_dict["sem_seg_file_name"], "L").squeeze(2)
            aug_input = T.AugInput(image, sem_seg=sem_seg_gt)
            transforms = self.augmentations(aug_input)
            image, sem_seg_gt = aug_input.image, aug_input.sem_seg
            image_shape = image.shape[:2]

            ret[k] = copy.deepcopy(dataset_dict)
            ...
            ret[k]['instances'] = self._transform_annotations(ret[k], transforms, image_shape)
            # ret[k].pop('annotations')
            if k == 'ref':
               
                flow_gt = cv2.imread(v.pop("flow_file_name"), cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH).astype(np.float64)  # !!! unavoidable bug
                ...
                if 'depth_file_name' in dataset_dict:
                    depth_gt = utils.read_image(dataset_dict["depth_file_name"]).astype(float)
